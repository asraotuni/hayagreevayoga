...
form.addEventListener("submit", async (event) => { event.preventDefault(); if (!form.reportValidity()) return; status.hidden = false; status.textContent = "Saving your profile…"; try { await (await getAuth()).updateUserAttributes({ userAttributes: { given_name: fields["profile-first-name"].value.trim(), family_name: fields["profile-last-name"].value.trim() } }); localStorage.setItem(profileKey, JSON.stringify({ birthdate: fields["profile-dob"].value, phone: fields["profile-mobile"].value.trim(), address: fields["profile-address"].value.trim(), country: fields["profile-country"].value.trim() || "India", state: fields["profile-state"].value.trim(), pin: fields["profile-pin"].value.trim() })); location.assign("/"); } catch (error) { status.textContent = error.message || "Unable to save profile."; } });
